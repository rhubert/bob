    ...
    def _generateSbomInfos(self, step, rootBomRef=None, processed=[], deployed={}):
        ...
        if 'sbom_deployed' in audit_files:
            ...
            for e in sbom_deployed:
                next_deployed[bytes.fromhex(e['bob:variantId'])] = e['files']
...
class CycloneDXGenerator(SBOMGeneratorBase):
    ...
    def _generate_component(self, info: ArtifactInfo, deployed):
        ...
        component = {
            ...
            "name": info.name,
            "cpe": info.cpe,
            "licenses": CycloneDXGenerator._generate_licenses(info),
            "externalReferences": []
        } | SBOMGeneratorBase.append_if_set(info, 'version')
    ...
    def _generate_licenses(artifact_info: ArtifactInfo):
        ...
        if artifact_info.license:
            return { "licenses" : [ { "license": { "id": artifact_info.license } } ] }
        return {}

class SBOMGenerator:
    def __init__(self, config: SBOMGeneratorConfig):
        self.__config = config

    def generate(self, step, sbomPath):

        generator = CycloneDXGenerator(step, self.__config)
        self._sbom = generator.generate()

        dump_args = {}
        if self.__config.pretty:
            dump_args['indent']=2
            dump_args['sort_keys']=True

        try:
            with gzip.open(sbomPath, 'wb', 6) as gzf:
                json.dump(self._sbom, io.TextIOWrapper(gzf, encoding='utf8'), **dump_args)
        except IOError as e:
